...
Confirmation.prototype = {
    ...
    init: function() {
        ...
        $(document).on('click', 'a', function(event) {
            if ($(this).attr('data-confirmation')) {
                T.link = $(this);
                T.showDialog();
                if (!($(this).attr('data-is-only-notice') == "1")) { 
                    event.preventDefault();
                    return false;
                }
            }
    ...
    showDialog: function() {
        ...
        if (T.link.attr('data-is-only-notice') == "1") { 
            var dialog = this.noticeDialog(message);
        } else {
...
LOCAL_TARICS = {
	...
	_index_search_show_administrative_end_administrative: {
		search_item_actions: ["on_ready"],
		trigger_focus_on_searchfield: ['on_ready', 'on_reload', 'on_change', 'on_resize'],
		local_tarics_validations: ["on_reload"]
	}
}
;
...
SESSIONS = {
    ...
    _ALL: {
        remove_validation_errors: ['on_reload', 'on_change']
    },

    new: {
        supress_html5_bubbles: ['on_ready']
    }
}
;
MANUFACTURERS = {
	...
	_new_edit_update_create: {
        toggle_edit_other_details: ['on_reload']
	}
}
;
